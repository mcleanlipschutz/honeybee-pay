...
| --- | --- | --- |
| M1 — contract foundation | Compact invoice commitment, owner witness authentication, exact-term matching, one-time consumption, Apache-2.0 scope, simulation and documentation | Full compiler completes with both circuit key sets; adversarial tests pass. Implemented in this branch. Mobile request encoding now has three additional passing adapter tests. |
...
| M3 — checkout connection | Define exact invoice/address/asset encoding, bind the expected contract and owner, review/confirm UI, timeout/recovery and truthful status; keep the approval action distinct from money movement | A mobile-request-to-Compact adapter is implemented and tested in local simulation. Browser proof linkage remains incomplete. |
...
| M5 — Wave 1 evidence | Reproducible README, progress delta, public commit, GitHub `midnightntwrk` topic, short pitch deck and recorded demo | Pitch deck, recorded-output demo video, progress description and midnightntwrk PR label prepared. Personal submission and device checks remain. |
