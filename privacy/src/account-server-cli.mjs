...
try {
  ...
  const server = await startAccountServer({ directory: await realpath(path), appId, verificationKey,
    ...
    onSyncDiagnostic: ({ stage, reason }) => console.error(`[Private wallet] ${stage}: ${reason}`),
    port: Number(process.env.HONEYBEE_ACCOUNT_PORT || 4173) });
  console.log(`Honeybee local testnet demo: ${server.origin}`);
  console.log('Experimental Sepolia build. Local runtime handles test-wallet keys. Deposits and private payments require separate confirmation.');
