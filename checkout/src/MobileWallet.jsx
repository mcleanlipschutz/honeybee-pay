...
export function MobileWallet({ connection, local = false }) {
  ...
  useEffect(() => {
    if (!active) return;
    let alive = true;
    const reload = () => { if (document.hidden) return; loadWalletBalance(rpc, address).then(data => { if (alive) { setBalance(data); setBalanceError(false); } }).catch(() => { if (alive) { setBalance(null); setBalanceError(true); } }); };
    reload(); window.addEventListener('focus', reload); document.addEventListener('visibilitychange', reload);
    return () => { alive = false; window.removeEventListener('focus', reload); document.removeEventListener('visibilitychange', reload); };
  ...
  useEffect(() => {
    // One automatic recovery check after sign-in or refresh; no transaction is sent here.
    if (!address) return;
    let live = true;
    try { const saved = readAttempt(localStorage, address); if (saved?.status === 'pending') inspectAttempt(rpc, saved).then(async result => { if (live && result && navigator.locks) await navigator.locks.request(attemptKey(address), () => { if (!live) return; const latest = commitAttemptResult(localStorage, address, result); setAttempt(latest); setRefresh(v=>v+1); }); }).catch(()=>{}); } catch {}
    return () => { live = false; };
  }, [address]);
  let parsedInput;
  ...
  return <div className="mobile-shell"><header className="app-header"><a className="brand" href="/" aria-label="Honeybee Pay home"><img src="/assets/honeybee-mascot.png" alt="" width="40" height="40"/><span>honeybee<span className="brand-pay">pay</span></span></a>{active && <button className="avatar" aria-label="Your account" disabled={busy} onClick={()=>{setAccount(v=>!v);setMessage('');}}>{connection.email?.[0]?.toUpperCase() || 'H'}</button>}</header><main className="mobile-main"><div className="test-label"><span/>Test mode · No real money</div>
    {signedOut ? <section className="welcome"><div className="welcome-bee"><img src="/assets/honeybee-mascot.png" alt="Honeybee" width="160" height="160"/></div><p className="eyebrow">A LITTLE LESS COMPLICATED.</p><h1>Money moves.<br/>You get on<br/>with your day.</h1><p>One wallet. Simple payments.<br/>A little more honey.</p><button className="primary" disabled={!connection?.ready} onClick={()=>connection?.login()}>{!connection ? 'Sign-in is being set up' : connection.ready ? 'Continue with email' : 'Getting ready…'}<Icon name="pay"/></button><small>Try sending and receiving test USDC.<br/>These test payments are public.</small></section>
    : !active ? <section className="empty" aria-live="polite"><h1>Opening your wallet…</h1><p>This may take a moment the first time.</p><button className="text-button" onClick={()=>connection.logout()}>Sign out</button></section>
