...
function ConnectedCheckout({ runtime }) {
  ...
  return <LegacyCheckout key={authenticated && user?.id || 'signed-out'} runtime={runtime} connection={{ ready: ready && walletsReady, authenticated, userId: user?.id, email: user?.email?.address, getAccessToken, wallet, login, logout, sendTransaction }} />;
}
export function LegacyCheckout({ connection, runtime }) {
  ...
  return <div className={`shell${localWalletSetup && view === 'wallet' ? ' local-wallet-shell' : ''}`}>
    <header><a className="brand" href="/" aria-label="Honeybee Pay home"><img className="mark" src="/assets/honeybee-mascot.png" alt="" width="52" height="52" decoding="async"/>honeybee<span className="brand-light">pay</span></a><span className="network"><i/>Sepolia testnet</span></header>
