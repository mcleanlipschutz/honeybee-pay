...
export const shieldAmountUnits = testAmountUnits;
...
export function validateShieldReview(value, { amount, publicAddress, walletId, privateAddress, assetId = 'usdc' }, now = Date.now()) {
  const review = structuredClone(value), asset = testAsset(assetId), token = asset.token;
  ...
  if (review.version !== 1 || review.status !== 'prepared-not-submitted' || review.submissionEnabled !== false
      || review.network !== 'Ethereum_Sepolia' || review.chainId !== 11155111 || review.token !== token
      ...
      || amountUnits !== shieldAmountUnits(amount, assetId)
      || received <= 0n || received + fee !== amountUnits || feeBps >= 10000n
      || fee !== amountUnits * feeBps / 10000n || (asset.id === 'usdc' && units(review.publicBalanceUnits) < amountUnits)
  ...
  if (preimage.token.tokenType !== 0 || getAddress(preimage.token.tokenAddress) !== token
      || preimage.token.tokenSubID !== 0n || preimage.value !== amountUnits) throw new Error('Deposit terms changed');
  if (review.approvalRequired) {
    const approval = transaction(review.approval, token, approvalAbi);
